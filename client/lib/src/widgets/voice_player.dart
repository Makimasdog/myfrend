...
class _VoicePlayerBubbleState extends State<VoicePlayerBubble> with SingleTickerProviderStateMixin {
  ...
  Future<void> _toggle() async {
    if (_isPlaying) {
      ...
      if (d != null && mounted) setState(() => _maxDuration = d.inMilliseconds.toDouble());
    }
  }
  ...
  String get _baseUrl {
    ...
    // 如果是相对路径，拼接 API 基础地址
    return 'http://192.168.1.5:3000${widget.voiceUrl}';
  }
    ...
    return Container(
      ...
      child: InkWell(
        ...
        onTap: () async {
          ...
          try {
            await _player.play(UrlSource(_baseUrl));
            final d = await _player.getDuration();
            if (d != null && mounted) setState(() => _maxDuration = d.inMilliseconds.toDouble());
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              Text(
                _isPlaying
                    ...
                    : (widget.duration != null ? '${widget.duration}s' : (_maxDuration > 0 ? '${(_maxDuration / 1000).toStringAsFixed(1)}s' : '语音消息')),
...
class _ProgressWavePainter extends CustomPainter {
  ...
  _ProgressWavePainter({required this.progress, required this.color, required this.playedColor});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < barCount; i++) {
      ...
      final p = i / barCount;
      final height = (3 + (p < 0.5 ? p * 2 : (1 - p) * 2) * 14).clamp(3.0, size.height - 4);
      paint.color = p <= progress ? playedColor : color;
      canvas.drawLine(Offset(x + barWidth / 2, size.height / 2 - height / 2), Offset(x + barWidth / 2, size.height / 2 + height / 2), paint);
    }
  }

  @override
  bool shouldRepaint(covariant _ProgressWavePainter old) => old.progress != progress;
}
