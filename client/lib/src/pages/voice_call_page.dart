...
class _VoiceCallPageState extends State<VoiceCallPage>
  ...
  void initState() {
    ...
    _pulseAnim = Tween(begin: 1.0, end: 1.4)
        .animate(CurvedAnimation(parent: _pulseCtrl, curve: Curves.easeInOut));
    _pulseCtrl.repeat(reverse: true);
    _waveCtrl = AnimationController(
        vsync: this, duration: const Duration(milliseconds: 800));
    _waveAnim = Tween(begin: 0.0, end: 1.0)
  ...
  Future<void> _initCall() async {
    ...
    try {
      final available = await _speech.initialize(
        onStatus: (status) {
          if (!_disposed && status == 'notListening') _startListening();
        },
  ...
  void _connectWebSocket() {
    ...
    try {
      final uri = widget.api.webSocketUri('/ws/call', queryParameters: {
        'friend_id': widget.friendId,
      });
      debugPrint('[Call] WS connected');
      _channel = WebSocketChannel.connect(uri);
      _channel!.sink
          .add(jsonEncode({'type': 'auth', 'token': widget.api.token}));
      setState(() {
      ...
      _channel!.stream.listen(
        ...
        onError: (e) {
          ...
          setState(() {
            _connState = 'failed';
            _statusText = 'Connection lost, retrying...';
          });
        ...
        onDone: () {
          ...
          if (!_disposed) {
            setState(() {
              _connState = 'failed';
              _statusText = 'Disconnected, retrying...';
            });
          }
          Future.delayed(const Duration(seconds: 3), () {
            if (!_disposed && _connState == 'failed') _connectWebSocket();
  ...
  void _handleMessage(Map<String, dynamic> msg) {
    ...
    switch (msg['type']) {
      ...
      case 'recognized':
        setState(() {
          _recognizedText = msg['text'] ?? '';
          _statusText = 'Heard: ${msg['text']}';
        });
        break;
      case 'tts_start':
        setState(() {
      ...
      case 'text_reply':
        setState(() {
          _aiText = msg['text'] ?? '';
          _statusText = 'AI replied (text)';
        });
        break;
      case 'error':
        setState(() {
          _statusText = 'Error: ${msg['error']}';
          _aiText = msg['error']?.toString() ?? '';
        });
        break;
    }
  }
          ...
          onResult: (result) {
            ...
            if (result.finalResult) {
              ...
              if (text.isNotEmpty) {
                ...
                _channel?.sink
                    .add(jsonEncode({'type': 'speech', 'text': text}));
          ...
          child: Column(children: [
            ...
            if (_recognizedText.isNotEmpty || _aiText.isNotEmpty)
              Container(
                ...
                child: Text(_isAiSpeaking ? _aiText : _recognizedText,
                    textAlign: TextAlign.center,
                    style: TextStyle(
...
class _CallWavePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    const bars = 30;
    final barW = size.width / (bars * 1.5);
    for (int i = 0; i < bars; i++) {
      final x = i * barW * 1.5;
      double h;
      if (i < bars / 3) {
        h = (i / (bars / 3)) * size.height * 0.8;
      } else if (i < bars * 2 / 3) {
        h = size.height * 0.8;
      } else {
        h = (1 - (i - bars * 2 / 3) / (bars / 3)) * size.height * 0.8;
      }
  ...
  @override
  bool shouldRepaint(covariant _CallWavePainter old) =>
