...
class _ChatPageState extends State<ChatPage> {
  ...
  Future<void> _init() async {
    ...
    if (sid == null) {
      final s =
          await chat.getOrCreateSession(widget.friendId, widget.friendType);
      sid = s?.id;
    }
    if (sid != null && mounted) {
      setState(() => _sid = sid);
  ...
  Future<void> _send() async {
    ...
    final chat = context.read<ChatProvider>();
    final sent = await chat.sendMessage(_sid!, text);
    if (sent == null) {
  ...
  Future<void> _streamAiReply() async {
    ...
    try {
      String fullText = '';
      await for (final data in api.streamAiReply(sessionId)) {
        final token = data['token'];
        if (token is String) {
          fullText += token;
          tempMsgId ??= 'streaming_${DateTime.now().millisecondsSinceEpoch}';
          if (fullText == token) {
            chat.addStreamingMessage(sessionId, tempMsgId, fullText);
          } else {
            chat.updateStreamingMessage(sessionId, tempMsgId, fullText);
          }
          _scrollToBottom();
        }
        if (data['error'] != null) {
        ...
        if (data['done'] == true) {
          ...
          if (message is Map) {
            ...
            if (tempMsgId == null) {
              chat.replaceStreamingMessage(sessionId, '', parsed);
            } else {
              chat.replaceStreamingMessage(sessionId, tempMsgId, parsed);
            }
          }
          completed = true;
          break;
      ...
      if (!completed) {
        throw ApiException('AI reply ended unexpectedly');
      }
    } catch (e) {
      if (tempMsgId != null) {
        chat.removeMessage(sessionId, tempMsgId);
      }
      final reply = await chat.getAiReply(sessionId);
  ...
  Future<void> _onVoiceDone(Map<String, dynamic> result) async {
  ...
  void _showAdvisor() {
    ...
    showModalBottomSheet(
      ...
      builder: (ctx) =>
          _AdvisorSheet(sessionId: _sid!, friends: afProvider.friends),
    );
  }
      ...
      appBar: AppBar(
        title: Row(children: [
          ...
          Expanded(
              child: Column(
                ...
                Text(
                    widget.friendType == 'ai'
                        ? (_replying ? '正在输入...' : 'AI 朋友')
                        : '在线',
        ...
        actions: [
          ...
          if (widget.friendType == 'human')
            IconButton(
                icon: Icon(Icons.lightbulb_outline, color: AppTheme.accent),
                tooltip: 'AI军师',
...
String _formatMsgTime(String dt) {
  try {
...
class _Bubble extends StatelessWidget {
  ...
  final String friendName;
  const _Bubble(
      {required this.msg, required this.cs, required this.friendName});
                  ...
                  children: [
                    ...
                    if (msg.createdAt != null)
                      Padding(
                          ...
                          child: Text(_formatMsgTime(msg.createdAt!),
                              style: TextStyle(
                                  fontSize: 10,
                                  color: isMe
                                      ? cs.onPrimary.withAlpha(120)
...
class _InputBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration:
          BoxDecoration(color: cs.surface, boxShadow: AppTheme.shadowSm(cs)),
      child: SafeArea(
        child: Row(crossAxisAlignment: CrossAxisAlignment.end, children: [
          VoiceRecordButton(
              api: context.read<ApiService>(), onUploaded: onVoiceUploaded),
          Expanded(
              child: Container(
...
class _AdvisorSheetState extends State<_AdvisorSheet> {
  ...
  Future<void> _getAdvice() async {
    ...
    try {
      ...
      final result = await svc.getAdvice(widget.sessionId, _selectedId!,
          context: _ctxCtrl.text);
      if (mounted) {
        setState(() {
    ...
    return DraggableScrollableSheet(
        ...
        builder: (_, scrollCtrl) => Padding(
            ...
            child: ListView(controller: scrollCtrl, children: [
              ...
              Text('🧠 AI 军师', style: Theme.of(context).textTheme.titleLarge),
              const SizedBox(height: 6),
              Text('选一个AI朋友帮你分析聊天，给出建议',
              ...
              SizedBox(
                  ...
                  child: FilledButton(
                      onPressed:
                          (_selectedId == null || _loading) ? null : _getAdvice,
                      child: _loading
                          ? const SizedBox(
                              width: 20,
                              height: 20,
