...
class CreateAiFriendPage extends StatefulWidget {
  ...
  const CreateAiFriendPage({super.key, this.friend});
  @override
...
class _CreateAiFriendPageState extends State<CreateAiFriendPage> {
  ...
  void initState() {
    ...
    _nameCtrl.text = friend.name;
    _gender = friend.gender;
    _ageRange = friend.ageRange ?? '';
    _personality = friend.personality ?? '';
    final extra = friend.extraConfig ?? const <String, dynamic>{};
    _hobby = extra['hobbies']?.toString() ?? '';
    _style = extra['speakingStyle']?.toString() ?? '';
    _role = extra['relationshipRole']?.toString() ?? '';
    _extraCtrl.text = extra['note']?.toString() ?? '';
  }
  ...
  Future<void> _submit() async {
    ...
    final friend = _isEditing
        ? await p.updateFriend(widget.friend!.id, payload)
        : await p.createFriend(payload);
    if (mounted) {
      setState(() => _creating = false);
      if (friend != null) {
        ScaffoldMessenger.of(context)
            .showSnackBar(SnackBar(content: Text('${friend.name} 创建成功！')));
        ...
        Expanded(
          ...
          child: FilledButton(
            ...
            child: _creating
                ...
                : Text(_step == _stepLabels.length - 1 ? '创建' : '下一步',
  ...
  Widget _buildStep(ColorScheme cs) {
    switch (_step) {
      ...
      case 1:
        return _genderStep(cs);
      case 2:
        final chips = _ages
            .map((a) => ChoiceChip(
                label: Text(a),
                selected: _ageRange == a,
                onSelected: (_) => setState(() => _ageRange = a),
                selectedColor: cs.primaryContainer,
                padding:
                    const EdgeInsets.symmetric(horizontal: 16, vertical: 10)))
            .toList();
        return _centerStep(Icons.calendar_today, '你希望TA的年龄在什么区间？',
            Wrap(spacing: 10, runSpacing: 10, children: chips));
      case 3:
        return _pickList(cs, '你希望TA是什么样的人？', _traits, _personality,
            (v) => setState(() => _personality = v));
      case 4:
        return _pickList(cs, 'TA有什么兴趣爱好？', _hobbies, _hobby,
            (v) => setState(() => _hobby = v));
      case 5:
        return _pickList(
            cs, 'TA的说话风格？', _styles, _style, (v) => setState(() => _style = v),
            extra: TextField(
      ...
      case 6:
        return _pickList(
            cs, 'TA在你生活中的角色？', _roles, _role, (v) => setState(() => _role = v));
      default:
              ...
              child: SizedBox(
                ...
                child: FilledButton.tonal(
                  ...
                  style: FilledButton.styleFrom(
                    backgroundColor: _gender == g.value
                        ? cs.primaryContainer
                        : cs.surfaceContainerHighest,
    ...
    return Builder(builder: (context) {
      ...
      return Center(
          child: Padding(
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                Icon(icon, size: 64, color: cs.primary.withAlpha(120)),
  ...
  static Widget _pickList(ColorScheme cs, String title, List<String> items,
        ...
        Card(
          ...
          child: ListTile(
              title: Text(t,
                  style: TextStyle(
                      fontWeight:
                          selected == t ? FontWeight.w700 : FontWeight.w400)),
              trailing: selected == t
