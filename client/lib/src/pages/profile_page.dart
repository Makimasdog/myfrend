  ...
  Future<void> _save() async {
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('资料已更新'), behavior: SnackBarBehavior.floating));
    }
  }
  ...
  Future<void> _saveLlmConfig() async {
    ...
    if (ok == true) {
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              content: Text('LLM 配置已保存'), behavior: SnackBarBehavior.floating));
        }
      } catch (e) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
              content: Text('保存失败: $e'), behavior: SnackBarBehavior.floating));
        }
      }
    }
    baseUrlCtrl.dispose();
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.all(20), children: [
      ...
      SegmentedButton<String>(
        segments: const [
          ButtonSegment(
              value: 'male', label: Text('♂'), icon: Icon(Icons.male)),
          ButtonSegment(
              value: 'female', label: Text('♀'), icon: Icon(Icons.female)),
          ButtonSegment(
              value: 'other', label: Text('其他'), icon: Icon(Icons.transgender)),
        ],
        selected: {_gender},
      ...
      _SettingTile(
          ...
          title: 'LLM API 配置',
          subtitle: '自定义你的AI接口',
          onTap: _saveLlmConfig),
      _SettingTile(
          icon: Icons.security, title: '账号安全', subtitle: '修改密码', onTap: () {}),
      _SettingTile(
